...
- Add matching public-header directories under `include/motion_control/<component>/`. Each component's **public API surface** lives there; `#include <motion_control/<component>/<file>.hpp>` is the consumer pattern. Private/implementation headers (helpers, internal types, bus-frame parsers, plugin-registry guts) are co-located with the source files that use them under `src/<component>/`, so the public include path strictly reflects the public API and ABI hygiene is automatic.
